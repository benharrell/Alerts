...
- (void)viewDidLoad
{
    ...
    //TODO: this disabled custom until we get it working....
...
-(void) sizeMapToDefault
{
    ...
    if (loc == nil)
    {
        [self showStatusMessage:@"Unable to size map to last setting, using default size and location."];
        return;
    }
    
    CLLocationCoordinate2D coord = loc.coordinate;
    
    NSLog(@"sizeMapToDefault - last location pref value: %f, %f", coord.latitude, coord.longitude);
    
    CLLocationDistance span = [self.appDelegate.alertsManager getLastMapSizePreferenceValue] *METERS_PER_MILE;
    
    NSLog(@"sizeMapToDefault - last map size pref value: %f", span);
    ...
    {
        NSLog(@"sizeMapToDefault - something wrong with regionThatFits result");
        ...
        if (viewRegion.span.latitudeDelta==0
            ...
            || isnan(viewRegion.span.longitudeDelta))
        {
            NSLog(@"sizeMapToDefault - something wrong with backup make region result");
            [self showStatusMessage:@"Unable to size map to last setting, using default size and location."];
            return;
        }
        else
...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control
{
    ...
    else
    {
        ...
        UISwitch * onoff = (UISwitch *) control;
        if (onoff.on)
        {
            //then make sure its not in the ignore list
            [self.appDelegate.alertsManager removeIgnoreAlertItem:al];
        }
        else
        {
            //add it to the ignore list if not there
            [self.appDelegate.alertsManager addIgnoreAlertItem:al];
        }
...
-(void)didUpdateAlertLabel:(wdAlertLocation * )customAlertItem {
    ...
    [self.appDelegate.alertsManager updateAlertDetails:customAlertItem];
...
- (void)mapView:(MKMapView *)mapView didSelectAnnotationView:(MKAnnotationView *)view
{
    ...
    if (!al.isCustom)
    {
        UISwitch * onoff= (UISwitch *)view.rightCalloutAccessoryView;
        
        onoff.on = ! [self.appDelegate.alertsManager shouldIgnoreAlertItem:al];
    }
    else
    {
        //initialize anything for custom alerts callout???
    }
}
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id <MKAnnotation>)annotation {
    ...
    if ([annotation isKindOfClass:[wdAlertLocation class]]) {
        ...
        {
            CGRect frame = CGRectMake(0.0f, 0.0f, 40.0f, 30.0f);
            UISwitch * onoff= [[UISwitch alloc] initWithFrame:frame];
            
            onoff.on = [self.appDelegate.alertsManager shouldIgnoreAlertItem:al];
            annotationView.rightCalloutAccessoryView = onoff;
        }
